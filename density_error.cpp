...
int main(int argc, char **argv){
    ...
    for(i=0;i<ndata;i++){
        data_chi[i] = pow((schecter(logmh1[i],alphabf,mstarbf,phistarbf)-logphi[i])/logerrphi[i],2.0);
        fprintf(ofile,"%g %g %g \n",logmh1[i],data_chi[i],data_chi[i]/chisqmin);
    }
    fclose(ofile);
    free(ofilename);
    ...
    //3d-likelyhood
    double *lhood,lhood_max,lhood_tot;
    lhood = (double *)malloc(sizeof(double)*nbin*nbin*nbin);
    lhood_max = -1.0E80;
    lhood_tot = 0.0;
